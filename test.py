...
trace_id, c_span_id = zipkin_api.set_data(trace_id=trace_id, rpc_name="do sql", parent_span_id=span_id)
...
trace_id, c1_span_id = zipkin_api.set_data(trace_id=trace_id, rpc_name="do sql 1", parent_span_id=span_id)
zipkin_api.record_event('cr')  # Note duration is in microseconds, as defined by Zipkin
zipkin_api.record_key_value('lc', "123")  # You can use string, int, long and bool values
zipkin_api.submit_span(time()*1000*1000, 234)

trace_id, span_id = zipkin_api.set_data(trace_id=trace_id, rpc_name="exe sql", parent_span_id=c_span_id)
zipkin_api.record_event('cr')  # Note duration is in microseconds, as defined by Zipkin
zipkin_api.record_key_value('lc', "123")  # You can use string, int, long and bool values
zipkin_api.submit_span(time()*1000*1000, 1230)

trace_id, span_id = zipkin_api.set_data(trace_id=trace_id, rpc_name="exe sql", parent_span_id=c1_span_id)
zipkin_api.record_event('cr')  # Note duration is in microseconds, as defined by Zipkin
zipkin_api.record_key_value('lc', "123")  # You can use string, int, long and bool values
zipkin_api.submit_span(time()*1000*1000, 1230)
